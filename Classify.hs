...
getWordFeature (_, _, m) str = maybe (WordFeature 
                                      {word = str, 
                                       hamCount = 0, 
                                       spamCount = 0}) id (M.lookup str m)

extractFeatures :: WMap -> String -> [WordFeature]
extractFeatures m = map (getWordFeature m) . nub . words

train :: WMap -> String -> MsgType -> WMap
train m s t = let ws = nub $ concatMap words $ lines s in
    foldl (incrementCount t) m ws

incrementCount :: MsgType -> WMap -> String -> WMap
incrementCount Ham (hc, sc, m) s = 
    let mfeat = M.lookup s m 
        feat = maybe (getWordFeat s 1 0) 
...
incrementCount Spam (hc, sc, m) s = 
    let mfeat = M.lookup s m 
        feat = maybe (getWordFeat s 0 1) 
               (\wf -> getWordFeat s (hamCount wf) 
                       (spamCount wf + 1)) mfeat 
    in 
      (hc, sc+1, M.insert s feat m)

getWordFeat :: String -> Int -> Int -> WordFeature
getWordFeat w ham spam = WordFeature {word = w, hamCount = ham, spamCount = spam} 
